...
class AccessibilityScraper(BaseScraper):
        ...
        is_criteria_met = (
            ...
            if (
                (
                    "translate"
                    or "select language"
                    or "select-language" in self.page.text.lower()
                )
                or ("espanol" or "Espa&ntilde;ol") in self.page.a
            )
            else False
        )
    ...
    def get_site_performance(self):
        try:
            lighthouse_results = self.apiClient.get_page_insights(
                self.url, "performance"
            ).content["lighthouseResult"]
            performanceResults = lighthouse_results["categories"]["performance"][
                "score"
            ]
            is_criteria_met = (
                True if performanceResults * 100 >= 80 else False
            )  # the score in the Json file is a percentage
    ...
    def get_mobile_friendliness(self):
        try:
            ...
            is_criteria_met = (
                True
                if title == "Content is sized correctly for the viewport"
                else False
            )
    ...
    def get_page_speed(self):
        try:
            ...
            is_criteria_met = (
                True if speed_index * 100 >= 80 else False
            )  # the score in the Json file is a percentage
